...
async function POST({ request }) {
  try {
    ...
    const { data: numeroPedido, error: errorNumero } = await supabaseAdmin.rpc("generar_numero_pedido");
    ...
    if (body.cliente_whatsapp) {
      const { data: clienteExistente } = await supabaseAdmin.from("clientes").select("id").eq("whatsapp", body.cliente_whatsapp).single();
      if (clienteExistente) {
        ...
        const { data: nuevoCliente, error: errorCliente } = await supabaseAdmin.from("clientes").insert({
          nombre: body.cliente_nombre,
    ...
    const { data: pedido, error: errorPedido } = await supabaseAdmin.from("pedidos").insert(pedidoData).select().single();
    ...
    const { error: errorItems } = await supabaseAdmin.from("pedidos_items").insert(itemsData);
    if (errorItems) throw errorItems;
    const { data: pedidoCompleto } = await supabaseAdmin.from("pedidos").select(`
        *,
        items:pedidos_items(*)
...
async function PUT({ request }) {
  try {
    ...
    const { data, error } = await supabaseAdmin.from("pedidos").update(updateData).eq("id", body.id).select().single();
    if (error) throw error;
    return json({
      success: true,
      data,
