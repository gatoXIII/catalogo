...
async function ensureUniqueSlug(baseSlug, excludeId = null) {
  ...
  let counter = 1;
  while (true) {
    const query = supabase.from("productos").select("id").eq("slug", slug);
    if (excludeId) {
...
async function POST({ request }) {
  try {
    ...
    const { data, error } = await supabaseAdmin.from("productos").insert([productoData]).select(`
        *,
        categoria:categorias(id, nombre, slug)
      `).single();
    if (error) {
...
async function PUT({ request }) {
  try {
    ...
    const { data, error } = await supabaseAdmin.from("productos").update(updateData).eq("id", id).select(`
        *,
        categoria:categorias(id, nombre, slug)
      `).single();
    if (error) throw error;
